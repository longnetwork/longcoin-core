...
{
    std::cout <<
        "This program outputs Longcoin addresses and private keys from a wallet.dat file" << std::endl
        << std::endl
        << "Usage and options: "
        << std::endl
...
bool WalletUtilityDB::DecryptSecret(const std::vector<unsigned char>& vchCiphertext, const uint256& nIV, CKeyingMaterial& vchPlaintext)
{
    ...
    BOOST_FOREACH(const CKeyingMaterial vMKey, vMKeys)
    {
        if(!cKeyCrypter.SetKey(vMKey, chIV))
            continue;
        //std::cout << "DecryptKey DecryptSecret-> " << HexStr(vMKey) << std::endl;
...
bool WalletUtilityDB::DecryptKey(const std::vector<unsigned char>& vchCryptedSecret, const CPubKey& vchPubKey, CKey& key)
{
    ...
    //std::cout << "DecryptKey vchPubKey-> " << HexStr(vchPubKey) << std::endl;
...
{
    //std::cout << "getCryptedKey masterPass: " <<  masterPass << ", ";
    mPass = masterPass.c_str();
    CPubKey vchPubKey;
    ssKey >> vchPubKey;
    //std::cout << "vchPubKey: " <<  HexStr(vchPubKey) << ", ";
    CKey key;

    std::vector<unsigned char> vKey;
    ssValue >> vKey;
    //std::cout << "vKey: " <<  HexStr(vKey) << ", ";

    if (!Unlock()) {
        //std::cout << "Unlock: " <<  "false" << ", \n";
        return "";
    }

    if(!DecryptKey(vKey, vchPubKey, key)) {
        //std::cout << "DecryptKey: " <<  "false" << ", \n";
        return "";
    }
...
{
    unsigned int nID;
    //std::cout << "updateMasterKeys nID: " <<  HexStr(vKey) << ", ";
    ssKey >> nID;
    //std::cout << "updateMasterKeys nID: " <<  nID << ", ";
    CMasterKey kMasterKey;
    ssValue >> kMasterKey;
    //std::cout << "updateMasterKeys kMasterKey: " <<  HexStr(kMasterKey) << ", ";
