...
/** Тарифы, меньшие этого (в сатоши), считаются нулевыми (для ретрансляции, добычи и создания транзакций) */
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < ::minRelayTxFee.GetFee(nSize))
        {
			...
			// -limitfreerelay unit - это тысяча байт в минуту
			// По умолчанию ставка займет 1 месяц, чтобы заполнить 1 ГБ
...
void ThreadScriptCheck() {
    RenameThread("longcoin-scriptch");
...
bool static DisconnectTip(CValidationState& state, const Consensus::Params& consensusParams)
{
    ...
    BOOST_FOREACH(const CTransaction &tx, block.vtx) {
        ...
        if (tx.IsCoinBase() || !AcceptToMemoryPool(mempool, stateDummy, tx, false, NULL, false)) {
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    // Check timestamp
    if (block.GetBlockTime() > GetAdjustedTime() + 1 * 60 * 60) // 2 * 60 * 60
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Check proof of work // FixMe: Для быстрого алгоритма регулирования возможно потребуется не строгое равенство а с допустимым разбросом
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION)
    {
        ...
        if (pfrom->nVersion == 10300)
            pfrom->nVersion = 300;
        if (!vRecv.empty())
            vRecv >> addrFrom >> nNonce; // Этот адрес сам отправитель вставил (наверно из externalip) и мог вставить не свой адрес а например 0.0.0.0 а точный адрес с которого пришло в pfrom->addr
        if (!vRecv.empty()) {
            vRecv >> LIMITED_STRING(pfrom->strSubVer, MAX_SUBVERSION_LENGTH);
            pfrom->cleanSubVer = SanitizeString(pfrom->strSubVer);
        }

        // Отбиваем чужих пиров игноря отбивку для захардкоженых нод
        
        bool fromSeed=false;
        for (std::vector<CAddress>::const_iterator it = chainparams.FixedAddresses().begin(); it!=chainparams.FixedAddresses().end(); ++it) {
           if ( (CNetAddr)*it == (CNetAddr)pfrom->addr ) {fromSeed=true; break;} // Приводим к базовому классу, где оператор сравнения не учитывает порт
        }        
//            #warning DEBUG peers
//            printf("fromSeed=%d, SubVer=%s, ExpectedSubVer=%s \n",fromSeed,pfrom->cleanSubVer.c_str(),strBeginSubVersion.c_str());
//            printf("Peer: %s / %s / %s\n", addrFrom.ToString().c_str(), pfrom->addr.ToString().c_str(), pfrom->addrLocal.ToString().c_str());
        if( !fromSeed && (pfrom->cleanSubVer.find(strBeginSubVersion)!=0) )
        {
            LogPrintf("peer=%d using non-LONG SubVersion: %s; disconnecting\n", pfrom->id, pfrom->cleanSubVer.c_str());
...
//               #warning PEER Debug
//                printf("Push GETADDR: if (pfrom->fOneShot || pfrom->nVersion >= CADDR_TIME_VERSION || addrman.size() < 1000) \n");
...
//        printf("/^^^ vAddr.size=%lu ^^^\\ \n",vAddr.size());
        ...
        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            boost::this_thread::interruption_point();
//            #warning DEBUG peers
//            printf("recv ADDR: %s ",addr.ToStringIPPort().c_str());
            ...
            // Будем брать только наши порты (и пировские > 10000)
            bool fKnownPort=false;
            if( (addr.GetPort()>10000) || (addr.GetPort() == 8777 || addr.GetPort() == 8778 || addr.GetPort() == 18777 || addr.GetPort() == 18778 || addr.GetPort() == 8334) ) fKnownPort=true;
...
//        #warning DEBUG peers
//        printf("\\___ fOneShot=%d ___/ \n",pfrom->fOneShot);
        
        addrman.Add(vAddrOk, pfrom->addr, 1 * 60 * 60); // 2 * 60 * 60
        if (vAddr.size() < 1000)
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (pindexLast == NULL) // should never hit.
            return false;
...
//        #warning DEBUG peer
//        printf("msg.hdrbuf ver=%i, msg.vRecv ver=%i \n",msg.hdrbuf.GetVersion(),msg.vRecv.GetVersion());
...
bool SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (pto->nNextAddrSend < nNow) {
            ...
            {
//                #warning DEBUG peers
//                printf("send ADDR: %s ",addr.ToStringIPPort().c_str());
                ...
                if (fKnownPort && !pto->addrKnown.contains(addr.GetKey()))
                {
