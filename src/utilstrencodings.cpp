...
bool ParseFixedPoint(const std::string &val, int decimals, int64_t *amount_out)
{
    ...
    if(res>LLONG_MAX) return false;
    if(res<LLONG_MIN) return false;
    
    *amount_out=(int64_t)(res);
    return true;
}


bool Old_ParseFixedPoint(const std::string &val, int decimals, int64_t *amount_out)
{    
    int64_t mantissa = 0;
    int64_t exponent = 0;
        ...
        if(decimals>0)
        {
            if (ptr < end && val[ptr] >= '0' && val[ptr] <= '9')
            {
                while (ptr < end && val[ptr] >= '0' && val[ptr] <= '9') {
                    ...
                    ++ptr;
                    ++point_ofs;
                    if(point_ofs>=decimals) break;
                }
            } else return false; /* missing expected digit */
        }
        while (ptr < end && val[ptr] >= '0' && val[ptr] <= '9') ++ptr; // skip other decimals
    }
...
//            }
//        } else return false; /* missing expected digit */
        return false; // exponent not support  
    }
    if (ptr != end)
